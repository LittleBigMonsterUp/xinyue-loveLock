...
Page({
    ...
    onLoad: function (options) {
        ...
        this.getAllTypeLabel(function (parentLabel,childLabel) {
            ...
            console.log(" childLabel 数据", childLabel)
            console.log("privacyType 数据",options.privacy_type)
            console.log("privacyType 数据",options.project_name)
            that.setData({
    ...
    getAllTypeLabel: function (callback) {
        wx.request({
            ...
            url:"http://localhost:31005/Clock/getAllGradeCourse",
            success:function (response) {
                console.log(response.data)
                let parentLabel = response.data.lsitGrade;
                let childLabel  = response.data.listClbum;
    ...
    createPunchCardProject: function () {
        ...
        wx.request({
            ...
            url:"http://localhost:31005/Clock/createClock",
            method: "post",
            data: {
                project_name:that.data.projectName,   // 创建打卡名字
                clock_course_name:that.data.privacyType,  // 打卡课程名字
                type_label: that.data.labelNameString,
                creator_id: parseInt(app.globalData.userInfo.id)
            },
            success:function (response) {
               console.log("clock 创建返回数据：", response)
                switch (response.statusCode) {
                    case 200:  
                    // console.log("clock 创建成功进入200：");
                        ...
                        getInviteImgUrl.then(function (res) {
                            wx.hideLoading();
                            let param =
                                '?projectId=' + parseInt(response.data.id)
