...
Page({
     ...
     */
    data: {
        userInfo: wx.getStorageSync("userInfo"), // 当前使用小程序的用户信息
        showLoading: false, // 在未从服务器端获取到打卡圈子信息之前则显示加载动画&&空白页面
    ...
    onShow: function () {
        ...
        promise[0] = new Promise(function (resolve) {
            ...
            //    url: app.globalData.urlRootPath + 'index/User/checkUserIsAttend',
               url:"http://localhost:31005/ClockDetail/checkUser",
               method: 'post',
               data: {
               ...
               success:function (res) {
                   console.log("checkUserIsAttendRes",res.data)
                   clearTimeout(id);
                   switch (res.statusCode) {
                       case 200:
                           ...
                           console.log("checkUserIsAttendRes",checkRes === true ? 1 : 0);
                           resolve(true);
                           break;
                       default:
                           resolve(false);
        ...
        promise[1] = new Promise(function (resolve) {
            ...
            wx.request({
                ...
                url:"http://localhost:31005/ClockDetail/getProjectInfo",
                method: 'post',
                data: {
                    projectId: that.data.projectId,
                    userId: app.globalData.userInfo.id
                ...
                success:function (res) {
                    console.log("projectInfo11111111",res)
                    clearTimeout(id);
                    switch (res.statusCode) {
                        case 200:
                            ...
                            that.setData({
                                'projectInfo.project_name': data.className,
                                'projectInfo.cover_img_url' : data.imgUrl,
                                'projectInfo.IntrInfoList' : data.intrInfoList,
                                'projectInfo.attendUserNum' : parseInt(data.attendUserNum),
                                'projectInfo.allPunchCardNum' : parseInt(data.allPunchCardNum),

                                'creatorInfo.creator_id' : data.creatorId,
                                'creatorInfo.nick_name' : data.nickName,
                                'creatorInfo.sex' : parseInt(data.sex),
                                'creatorInfo.avatar_url' : data.avatarUrl,
                                'creatorInfo.introduce' : data.introduce,
                                'creatorInfo.weChat' : data.weChatNum,

                                attendUserInfo: data.attendUserInfo,
                                isCreator: isCreatorFlag ? 1:0 // 1为创建者
                            });
                            console.log(" let data", data);
                            console.log("attendUserInfo",that.data.attendUserList);
                            resolve(true);
                            break;
                        default:
                            resolve(false);
    ...
    getPunchCardDiaryList: function(pageNo,dataNum) {
        ...
        wx.request({
            ...
            url:'http://localhost:31005/ClockDiary/getClockDiaryInfoList',
            method: 'post',
            data: {
            ...
            success: function (res) {
                ...
                switch (res.statusCode) {
                    case 200:
                        ...
                        console.log("onePageDiaryListData",that.data.onePageDiaryListData);
                        break;
                    default:
                        wx.showToast({
                            title: data.errMsg,
    ...
    dealUserLike: function(e) {
        ...
        if (haveLike === true)
        {
            ...
            wx.request({
               ...
               url:"http://localhost:31005/ClockDiary/clockDiaryInfoCancelLike",
                method: 'post',
                data: {
            ...
            console.log("触发点赞 diaryId",diaryId)
            // 进行点赞
            wx.request({
               // url: app.globalData.urlRootPath + 'index/DiaryLike/like',
               url:"http://localhost:31005/ClockDiary/clockDiaryInfoLick",
                method: 'post',
                data: {
                    diary_id: diaryId,
    ...
    attendPunchCardProject: function() {
        ...
        wx.request({
            ...
            url:"http://localhost:31005/ClockDetail/joinInProject",
            method: 'post',
            data: {
