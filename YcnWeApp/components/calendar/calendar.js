...
Component({
  ...
  methods: {
    ...
    selectDay(e) {
      ...
      let yaer = canlender.weeks[week][index].yaer
      console.log("canlender.weeks[week]", canlender.weeks[week][index])
      let obj =new Date();
      let nowyear=obj.getFullYear();
      let nowmonth=obj.getMonth()+1;
      let nowdate=obj.getDate();
     console.log("Number(yaer)===Number(nowyear)",yaer,month,date)
      // console.log("year<nowyear && month<nowmonth && date<nowday",year<nowyear && month<nowmonth && date<nowday?1:0)
      if(Number(month)<=Number(nowmonth) && Number(date) < Number(nowdate)){
        console.log("11111111")
        console.log(canlender.weeks[week][index].have)
        if(canlender.weeks[week][index].have===false){
          this.setData({
            buttonshow:"补卡"
          })
        }else{
    ...
    packup:function() {
      ...
      console.log("yaer1111",year);

      if (this.data.isOpen) {
        let year = self.data.canlender.year + "-" + self.data.canlender.month + "-" + self.data.canlender.date
        let _date = self.getDate(year, 0);
        self.getWeek(_date);
        if(self.data.buttonshow=='补卡'){
          wx.navigateTo({
            url: '/pages/punchCardDetailPage/publishPunchCardDiary/index',
          })
         }
        return
      }
      ...
      console.log("self.data.buttonshow=='补卡'",self.data.buttonshow=='补卡')
    },
    // 返回今天
    backtoday() {
      this.getWeek(new Date());
    ...
    //console.log("year,mouth,date",year,month,date);
